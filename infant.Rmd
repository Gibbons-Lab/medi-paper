...
meta <- fread("/proj/gibbons/2022_infant_data_st_louis/data/PRJNA473126.tsv")
meta <- meta[, .(
...
ggplot(totals[!is.na(total_reads)]) +
  ...
  aes(x=host_age_days, y=(reads + 1)/(total_reads + 1), color=bac/total_reads < 0.95) +
  scale_color_manual(values=c("black", "tomato")) +
  geom_point() +
  stat_smooth(data=totals[host_age_days >3], method="gam", col="blue") +
  labs(x="infant age [days]", y="fraction of food reads [reads + 1/total]") +
  scale_y_log10(label = scales::percent_format(scale=100)) + 
  guides(color=FALSE)
  
ggsave("figures/infant_total_food.pdf", dpi=300, width=6, height=4)
```
...
ggplot(cohorts[name == "Energy"]) +
  ...
  theme(axis.text.x = element_text(angle=45, vjust=1, hjust=1))
ggsave("figures/energy.pdf", width=2, height=3)
```
...
ggplot(detected, aes(x=age_group, y=detected/n)) +
  ...
  labs(x="age quantile [days]", y="food prevalence") +
  scale_y_continuous(labels=scales::percent_format(), limits=c(0, 1))
ggsave("figures/food_prevalence.pdf", width=6, height=2)
```

```{r}
ggplot(totals, aes(x=host_age_days, y=bac/total_reads, color=host_del_route)) +
  geom_point() +
  stat_smooth(method="gam") +
  labs(x="infant age [days]", y="bacterial reads [%total]", color="delivery route") +
  scale_y_log10()
ggsave("figures/bac_reads.png", dpi=300)
```
...
for (i in 1:length(tests)) {
  ...
  premod <- glm((abundance > 0) ~ as.numeric(group), data=dt)
  tables[[name]] <- dt
