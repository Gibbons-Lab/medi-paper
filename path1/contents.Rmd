...
ggsave("nutrients_48h.pdf", width=8, height=6)
```
...
If we assume that both medi and recall estimates are faithful measures of the abundance $x_i$ we can estimate the fraction
of total food making it into stool by the ratio of relative abundances:

$$
\frac{a_r}{a_m} = \frac{x_i}{total_{in}} / \frac{x_i\cdot\alpha_i}{total_{gut}} = \frac{total_{gut}}{total_{in} \alpha_i} 
$$

Here $\alpha_i$ is the adsorption fraction of the specific food. Obviously the highest adsorption is measured when $\alpha_i = 1$, so one can estimate it from the minimum mean ratio.

```{r}
adsorption <- means[, .(ratio=median(amount/abundance), ratio_sd=mad(amount/abundance)), by="measure"]
adsorption[, "specific_adsorption" := 1 - min(ratio)/ ratio]
adsorption
```

## Check for specific associations with foods

```{r}
foods <- fread("data/P4W1208.txt", skip = 1)
names(foods)[1:7] <- c("study", "Participant ID", "entry", "meal_file_id", "meal_time", "meal_name", "meal_location")
items <- names(foods)[8:ncol(foods)]
foods <- foods[merged[, .(sample_id, `Participant ID`, Period, time, Treatment, BaselineEnd)], on="Participant ID", nomatch=0]
foods[, "entry" := as.POSIXct(entry, format="%m/%d/%Y")]
foods[, "time_diff" := time - entry]
foods <- foods[BaselineEnd == "End"]
fwrite(foods, "data/foods_processed.csv")
foods
```

```{r}
ab <- fread("data/food_abundance.csv")
hib <- ab[food_id == 748]
hib <- hib[foods, on="sample_id", allow.cartesian = T][time_diff > 24]
hib
```